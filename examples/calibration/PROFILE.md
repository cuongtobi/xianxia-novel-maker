...
- direct, event-first xianxia narration;
- prose moves by cause → reaction → consequence → next decision;
...
Default paragraph movement:

`new fact/action → immediate effect → character/world response → changed situation`

A paragraph does not need to sound aphoristic or self-contained. Do not polish every paragraph into a punchline.
...
## 4. Character reasoning

Reasoning should be **decision-bound**.

Preferred pattern:

`observe → infer only what matters → compare risk/value → decide/act`
...
Exposition works best when attached to a live object/problem:

`encounter object/rule → inspect/test/read → discover property → assess benefit → discover cost/limit → revise plan`

A technical explanation may be long if it keeps producing new consequences. Avoid encyclopedia blocks that do not change present state.

Cultivation/resource exposition should answer practical questions such as:

- what can this do now?
- what does it cost?
- what risk appears?
- what new option opens or closes?
- why does the character care at this moment?

## 6. Combat mechanics

Preferred combat loop:

`attack/technique → defense/counter → collision → concrete physical/spatial result → tactical consequence → escalation or reversal`

Combat should remain spatially readable. Show what was struck, displaced, broken, blocked, lost or exposed.

Power scale may be demonstrated through:

- terrain/environment damage;
- bodily result;
- resource consumption;
- inability/ability to break defense;
- a knowledgeable observer's brief reaction;
- a new technique forcing a positional change.

Escalation should come from changed tactics, revealed capability, resource use, transformation, terrain or injury — not merely a longer list of skill names.

Sound/impact lines and witness reactions may punctuate a major exchange, but should not appear after every move.
...
- concrete verbs and nouns over adjective stacks;
...
- survival pressure;
- cautious tactical reading of stronger threats;
- action interrupted by world-scale consequence;
- brief world zoom-out followed by return to protagonist;
- pragmatic dialogue between cultivators;
- reasoning that ends in a concrete decision.

### Sample 2 signal

Use for:

- trap/formation/tactical combat;
- inspecting loot, techniques, pills, artifacts;
- stepwise technical discovery;
- benefit-versus-cost reasoning;
- changing a plan after learning a hidden limitation;
- calm protagonist behavior under pressure.
...
- high-intensity duel;
- attack/counter/impact loops;
- environmental destruction as power proof;
- escalation through new forms/techniques;
...
- Chinese clause order in Vietnamese;
- awkward pronoun repetition;
- punctuation/spacing errors;
- repeated connector tics such as mechanically using `lập tức / không ngờ / lúc này / sau đó` in every paragraph;
...
`state + character state + minimal canon anchors + selected calibrated/project examples`

As the project accumulates strong user-approved finals, project-owned examples take priority over these framework calibration examples.
